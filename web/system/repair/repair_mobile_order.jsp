<%-- 
    Document   : repair_mobile_order
    Created on : 22 เม.ย. 2556, 18:31:56
    Author     : Admin
--%>
...
<script type="text/javascript">
    $(function(){
        ...
        $("#repair_mobile_id").css("background-color","orange").css("color","black");
        ...
        $("#get_option_topic_repair_mobile").load("controller.manage_data_topic_repair_mobile?get_option");
        ...
        $("#cmd_save").click(function(){
            $.ajax({
                url:"controller.repair_mobile_order?save",
                type:"post",
                data: $("#repair_mobile_order").serialize(),
        ...
        $("#cmd_check").click(function(){
            $.ajax({
                url:"controller.repair_mobile_order?check",
                type:"post",
                data: $("#repair_mobile_order").serialize(),
                success:function(data){
                    if(data == "complete"){
                        ...
                        alert("จากการตรวจสอบพบว่ารหัสสินค้านี้ไม่เคยซื้อสินค้าจากทางร้าน หรือลูกค้าให้รหัสโทรศัพท์ที่เป็นข้อมูลเท็จ ดังนั้นไม่ว่าสินค้าจะมีอาการเป็นเช่นไรก็จะคิดค่าบริการตามความเหมาะสม");
                        clear_data();
                    }
                    else if(data == "data_null"){
                        alert("กรุณากรอกข้อมูลรหัสโทรศัพท์เพื่อทำการตรวจสอบว่าจะได้รับการซ่อมฟรีหรือไม่");
                        clear_data();
                    }
                    else{
                        alert(data);
    ...
    /*function clear_data(){
    $("#repair_mobile_id").val("");
    $("#sale_mobile_id").val("");
    $("#mobile_type_id").val("");
    $("#mobile_id").val("");
    $("#topic_repair_mobile_id").val("");
    $("#detail").val("");
    $("#price").val("");
    $("#status_id").val("");
    $("#date_get_product").val("");
    }
...
<%
    ...
    String sql = "SELECT count(repair_mobile_id) AS count_row FROM tb_repair_mobile_order";
...
<form id="repair_mobile_order">
    <legend>แจ้งการสั่งซ่อม</legend>
    <table>
        <tr>
            <td >รหัสการซ่อม</td>
            <td ><input  type ="text" name="repair_mobile_id" id="repair_mobile_id" class="input-mini" style=" padding: 5px;width: 40px;font-size: 15px ;margin: 3px" value="CF<%=session.getAttribute("i")%>"></td>
        </tr>
        <tr>
            <td>รหัสการขาย</td>
            <td colspan="2"><input type="text" name="sale_mobile_id" id="sale_mobile_id" class="input-mini" style="margin: 3px"><input type="button" id="cmd_check" class="btn btn-primary btn-large" value="ตรวจสอบข้อมูล" style="margin: 3px"></td>
        </tr>
        <tr>
            <td>ยี่ห้อ</td>
            <td id ="get_option_type_mobile"></td>
        </tr>
        <tr>
            <td>รุ่น</td><td id="get_option_mobile"></td>
        </tr>
        <tr>
            <td>อาการในประกัน</td>
            <td id="get_option_topic_repair_mobile"></td>
        </tr>
        <tr>
