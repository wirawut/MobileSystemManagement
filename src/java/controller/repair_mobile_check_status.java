...
public class repair_mobile_check_status extends HttpServlet {
    ...
    private void check(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        String sql = "SELECT tb_manage_data_status.status FROM tb_repair_mobile_order "
                + "JOIN tb_manage_data_status "
                + "ON tb_manage_data_status.status_id = tb_repair_mobile_order.status_id "
                + "WHERE tb_repair_mobile_order.repair_mobile_id = '" + repair_mobile_id + "'";
        try {
